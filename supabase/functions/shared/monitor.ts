...
export class Monitor {
  ...
  private ping(type?: 'start' | 'fail' | 'success', message?: string) {
    ...
    console.log(`🔗 [${this.jobName}] Pinging healthcheck: ${url} (${type || 'success'})`);
    fetch(url, {
      method,
      headers: method === 'POST'
      ...
      console.warn(`⚠️ [${this.jobName}] Healthcheck ping (${type || 'success'}) failed:`, e);
    });
  }
    ...
    const body = {
      exception: {
        values: [{
          ...
          stacktrace: {
            frames: (error?.stack || '').split('\n').map((line: string) => ({ function: line.trim() })),
          },
        }]
      },
