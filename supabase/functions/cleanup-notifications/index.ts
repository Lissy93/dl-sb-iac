...
serve(async (req) => {
  ...
  try {
    ...
    logger.info(`Deleted ${deletedCount ?? 0} notifications older than 30 days (${cutoff})`);
    ...
    const msg = `Cleanup completed: sent ${pending.length} pending notifications, and deleted ${deletedCount ?? 0} old notifications.`;
