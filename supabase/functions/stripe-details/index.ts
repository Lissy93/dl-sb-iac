...
serve(async (req) => {
  ...
  try {
    ...
    if (!userId && !customerId) {
      return jsonResponse({ error: 'Missing or invalid user or customer id' }, 400);
    }
    ...
    if (!customerId) {
      ...
      if (existingId) {
        ...
        return jsonResponse({ error: 'No customer ID found for this user' }, 404);
      }
    }
    ...
    // Fetch subscription
    try {
      const subRes = await fetch(`${STRIPE_BASE}/customers/${customerId}/subscriptions?limit=5&expand[]=data.items`, {
        headers,
      });
      ...
      if (subRes.ok && subscription) {
        ...
        result.discount = (subscription.discount && subscription.discount.coupon) ? {
    ...
    // Fetch past invoices
    try {
      const invRes = await fetch(`${STRIPE_BASE}/invoices?customer=${customerId}&limit=10`, { headers });
    ...
    try {
      const upRes = await fetch(`${STRIPE_BASE}/invoices/upcoming?customer=${customerId}`, { headers });
    ...
    try {
      const pmRes = await fetch(`${STRIPE_BASE}/payment_methods?customer=${customerId}&type=card`, { headers });
